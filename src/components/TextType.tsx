...
const TextType: React.FC<TextTypeProps> = ({
  ...
  const renderTextWithGradient = (text: string) => {
    // Apply gradient to specific words: "2025" and "LinkedIn"
    const gradientWords = ['2025', 'LinkedIn'];
    let result = text;
    ...
    const executeTypingAnimation = () => {
      if (isDeleting) {
        ...
        if (currentCharIndex < processedText.length) {
          ...
          // Single text completed, no loop - hide cursor
          setIsCompleted(true);
        }
      }
    };
  ...
  return createElement(
    ...
    <span 
      ...
      dangerouslySetInnerHTML={{ __html: renderTextWithGradient(displayedText) }}
    />,
